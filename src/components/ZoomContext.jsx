...
export const ZoomProvider = ({ children }) => {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            ...
            if (currentUser) {
                try {
                    const response = await fetch('https://hello-belly-flask-1.onrender.com/get_zoom_token', { method: 'POST' });
    ...
    const checkIfDoctor = async (email) => {
        ...
        try {
            const response = await fetch(`https://hello-belly-flask-1.onrender.com/api/is_doctor?email=${email}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (data.is_doctor) {
                const doctorResponse = await fetch(`https://hello-belly-flask-1.onrender.com/api/doctor_by_email?email=${email}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
